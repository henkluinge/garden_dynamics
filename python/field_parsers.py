...
# def gdf_to_regular_pandas_inplace(gdf_trees):
...
def gdf_trees_to_regular_pandas(gdf):
    ...
    gdf = pd.concat([gdf, pxpypz], axis=1)
    gdf = gdf.drop(columns=["geometry"])
    return pd.DataFrame(gdf)


def gdf_trees_to_geodataframe(df, crs=None):
    # Create geometry from px, py (if present)
    if crs is None:
        crs = geo_parsers.get_crs_veeleerveen()
...
def write_gps_coordinates_to_toml(gdf_trees, filename="gdf_trees.toml", verbose=False):
    ...
    gdf_trees = gdf_trees_to_regular_pandas(gdf_trees)
    ...
    records = [{k: _to_plain(v) for k, v in rec.items()} for rec in records]
...
def load_gps_coordinates_to_toml(
    filename="gdf_trees.toml", crs=None, as_geodataframe=False
):
    """
    ...
    # Create geometry from px, py (if present)
    gdf = gdf_trees_to_geodataframe(df, crs=None)

    # Keep column order similar to original notebook
    cols = [
        c for c in ["type", "label", "px", "py", "pz", "geometry"] if c in gdf.columns
    ]
    gdf = gdf[cols]
    # if as_geodataframe:
...
def add_row_to_tree_list(
    gdf, px=None, py=None, pz=None, new_id=None, base_id=0, replace=False, **kwargs
):
    ...
    new_id: New index of the row to be added or replaced. If None, the row will be added.
...
def read_gps_coordinates_recorded_with_app(
    fname="datasets/boomgaard_plant_coordinaten/20251005-091825.kml",
    crs_veelerveen=None,
):
    """Early october, I did a recording of GPS coordintates of the trees using the GPSlogger app. This function reads and cleans the specifica dataset."""
    gdf_trees = gpd.read_file(fname)
    gdf_trees = gdf_trees.drop(
        columns=[
            "Description",
            "timestamp",
            "begin",
    ...
    d_translate = {"Apple": "Appelboom", "Beuk": "Beukenboom", "Kers": "Kersenboom"}
    ...
    gdf_trees = add_row_to_tree_list(
        gdf_trees, kind="Walnut", px=86.0, py=6.0, new_id=None, base_id=2
    )

    # Make available to sensor_fusion_examples/notebooks/ranging_measurements.ipynb
    write_gps_coordinates_to_toml(
        gdf_trees, filename="./data_cache/examples_gdf_trees.toml", verbose=True
    )
...
def read_gps_coordinates_recorded_with_app(
    ...
    """October 2025, I did a recording of GPS coordinates of the trees. This function reads and cleans the specific dataset."""
    gdf_trees = gpd.read_file(fname)
    gdf_trees = gdf_trees.drop(
        columns=[
            "Description",
    ...
    write_gps_coordinates_to_toml(
        gdf_trees,
        filename=geo_parsers.DATASETS / "examples_gdf_trees.toml",
        verbose=True,
    )
