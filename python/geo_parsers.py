...
def get_crs_veeleerveen(origin_lat=53.0579, origin_lon=7.12, alpha=-21):
    crs_veelerveen = CRS.from_proj4(
        f"+proj=omerc +lat_0={origin_lat} +lonc={origin_lon} +alpha={alpha} +k=1 +x_0=0 +y_0=0 +gamma=0 +ellps=WGS84 +towgs84=0,0,0,0,0,0,0"
    )
...
def get_kadaster_objects(
    kadastrale_object_ids=kadastrale_object_ids_own,
    crs_label="Amersfoort",
    ...
    db_percelen = db_percelen.rename(
        columns={
            r"kadastraleAanduiding|TypeKadastraleAanduiding|kadastraleGemeente|KadastraleGemeente|code": "gemeente_id",
    ...
    if kadastrale_object_ids_own is None:
        p_point = (
            db_percelen[db.identificatie.isin(kadastrale_object_ids_own)]
            .union_all()
            .centroid
        )
        db_percelen = db_percelen[
            db_percelen.distance(p_point) < distance_to_own_percelen
        ]

    # It's common in kadaster data to combine 'sectie' and 'perceelnummer' to a single label (e.g. N123).
    db_percelen["label"] = db_percelen["sectie"] + db_percelen["perceelnummer"].astype(
        str
    )
...
def get_perceel_coordinates(
    ...
    db = db.drop(
        columns=[
            r"typeGrens|TypeGrens|code",
            r"typeGrens|TypeGrens|waarde",
            "volgnummer",
            "code",
            "waarde",
            "beginGeldigheid",
    ...
    d_perceel_geometry = gpd.GeoSeries(
        name=perceel_id,
        index=[
            perceel_id,
        ],
...
def get_north_arrow(crs_veelerveen, p_shift_x=0, p_shift_y=0, xfact=1, yfact=1):
    ...
    geometry = [
        affinity.scale(g, xfact=xfact, yfact=yfact) for g in [up_arrow, letter_n]
    ]

    gdf_north = gpd.GeoDataFrame(
        index=[0, 1], crs=crs_amersfoort, geometry=geometry
    )  # Because north up.
...
def get_eelde_weather_data(fname="eelde_2021-2030/uurgeg_280_2021-2030.txt"):
    ...
    df["datetime"] = df.apply(
        lambda d: datetime.strptime(str(d.YYYYMMDD) + str(d.HH).zfill(2), r"%Y%m%d%H"),
        axis=1,
    )
    col_names = {
        "DD": "windrichting",
